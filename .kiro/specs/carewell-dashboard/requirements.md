...
> **令和8年度の変更（2026-09-20、PR #52）**: 令和8年度の出欠管理名簿には通し番号の列が無く、`serial_number` が常に 0 になるため、受講生一覧・グループ別受講生一覧・カード表示の「通し番号」カラムを「受講者番号」（`student_number`、A001 形式）の表示・並べ替えに置き換えた。上記 AC 1・3 と Requirement 18 の AC 3 の「通し番号」は、現在は「受講者番号」を指す。
