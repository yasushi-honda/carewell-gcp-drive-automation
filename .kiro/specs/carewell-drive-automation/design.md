...
doc_ref = db.collection(class_name).document(task_id).collection("documents").document(composite_key)
...
**決定**: 1ページ目はフレーム再読み込み後に5秒待機、2ページ目以降はページ遷移後に3秒待機を実装

**コンテキスト**:
- Carewell WebサービスはASP.NET WebFormsで実装されており、ページ遷移時に`__doPostBack`によるサーバーサイドページ再生成が発生する
- 「全て」タブクリック後、iframeがリロードされ、テーブルのレンダリングに遅延が発生する
- 現在は全クラスで提出数が100件未満（1ページのみ）だが、将来的に100件以上になると2ページ目以降の処理が必要になる
- 2025年1月時点で、Playwright TimeoutError（`tr.standard_grid_item`セレクター待機）が発生し、既存提出ファイル（40件）の検出に失敗していた

**代替案**:
1. **待機時間なし**: 最速だが、テーブルレンダリング前にセレクターを検索してTimeoutErrorが発生
2. **全ページで一律5秒待機**: シンプルだが、2ページ目以降で不要な待機時間が発生
3. **動的待機（セレクター出現まで無限待機）**: タイムアウトなしでエラーハンドリングが困難
4. **ページ番号で分岐した待機時間**: フレーム再読み込み（1ページ目）とページ遷移（2ページ目以降）のメカニズムの違いを考慮
...
**技術詳細**:
- **HTML構造**: `<select name="ctl00$ContentPlaceHolder1$GridView1$ctl21$ddlPagerTop" onchange="javascript:setTimeout('__doPostBack(...)')">`
- **ASP.NETポストバック**: `pagination_select.select_option(str(next_page))`実行後、サーバーでHTMLが再生成される
- **タイムアウト設定**: 30秒（デフォルト10秒から延長）でネットワーク遅延やサーバー負荷を考慮

**理由**:
- フレーム再読み込み（1ページ目）はiframe全体のリロードを伴うため、レンダリング時間が長い（5秒）
- ページ遷移（2ページ目以降）はASP.NETポストバックのみで、フレーム構造は維持されるため待機時間が短い（3秒）
- 実際の動作確認で、5秒待機により40件の提出ファイル検出に成功（修正前は0件検出）
- 2ページ目以降は未検証だが、同様のASP.NETメカニズムのため同様の待機が必要と判断

**トレードオフ**:
- 獲得: TimeoutError解消、提出ファイル検出の信頼性向上、将来的な100件超え対応
- 犠牲: 1ページ目で+5秒、2ページ目以降で+3秒/ページの処理時間増加

**検証結果（2025年1月20日時点）**:
- carewell-class01-task01: 40件の提出ファイルを検出（修正前は0件）
- Cloud Scheduler 14ジョブ全てで正常動作確認済み
- 2ページ目以降のテストは実施されていない（全クラスで100件未満）
...
interface PlaywrightAutomationEngineService {
  ...
  navigate_to_task(class_name: string, task_pattern: string): Result<Page, NavigationError>;
...
interface SubmissionCollectorService {
  collect_all_submissions(page: Page, class_name: string, task_id: string): Result<Submission[], CollectionError>;
}
...
- **Firebase Admin SDK**:
  ...
  - コレクション階層: `db.collection(class_name).document(task_id).collection("documents")`
...
interface DuplicationCheckerService {
  is_duplicate(composite_key: string, class_name: string, task_id: string): Result<boolean, CheckError>;
}
...
interface MetadataStoreService {
  save_metadata(submission: Submission, uploaded_file: UploadedFile, class_name: string, task_id: string): Result<void, SaveError>;
}
...
**事後条件**:
- Firestoreに`{class_name}/{task_id}/documents/{composite_key}`ドキュメントが作成される
- 全フィールドが保存される

**不変条件**:
- ドキュメントIDは常に複合キー
- コレクション階層は`{class_name}/{task_id}/documents`
...
- **Google Sheets API**:
  ...
  - シート作成:
    ...
    # 存在しない場合は作成
    if task_id not in sheets:
        request = {'addSheet': {'properties': {'title': task_id}}}
        ...
        service.spreadsheets().values().update(
            spreadsheetId=spreadsheet_id, range=f'{task_id}!A1:G1',
...
interface SheetsRecorderService {
  append_row(submission: Submission, uploaded_file: UploadedFile, spreadsheet_id: string, task_id: string): Result<void, AppendError>;
}
...
- ジョブごとにオフセット設定:
  ...
  - ... (5分刻みで最大12ジョブ/30分まで配置可能)

**注意**: このシステムとは別に、申し込み状況取得用の7ジョブ（`carewell-automation-pattern1-5, 8-9`）が別のCloud Runサービス（`carewell-automation`）を呼び出している。これらは**絶対に削除・停止しないこと**。
...
3. **特定クラス・課題の処理履歴**:
   ...
   jsonPayload.task_id="課題①業務分析　※～11/3〆切"
   ```
...
ウィジェット:
  ...
  - タイトル: エラー発生状況
    ...
    カラム: timestamp, class_name, task_id, error_type
```
...
|---------|-----------|------------------|------|
| Cloud Scheduler（ファイル収集14ジョブ） | 14ジョブ × $0.10 = $1.40 | $1.40 | $0 |
| Cloud Scheduler（申し込み状況7ジョブ）※別システム | 7ジョブ × $0.10 = $0.70 | $0.70 | $0 |
| Cloud Run（ファイル収集） | 14ジョブ × 48回/日 × 30日 × 3分 × $0.0000025/GB-sec × 2GB = $0.60 | 14 × 32回/日 × 30日 × 3分 × $0.0000025/GB-sec × 2GB = $0.40 | $0.20 |
| Cloud Run（申し込み状況）※別システム | 7ジョブ × 48回/日 × 30日 × 2分 × $0.0000025/GB-sec × 2GB = $0.30 | 7 × 32回/日 × 30日 × 2分 × $0.0000025/GB-sec × 2GB = $0.20 | $0.10 |
| Firestore | 読み取り: 1,000件/日 × 30日 = 30,000件 → $0 (無料枠50,000件/日) | 同左 | $0 |
| **合計** | **$3.00/月（約450円）** | **$2.70/月（約405円）** | **$0.30/月（約45円）** |

**注**: 合計21ジョブ（ファイル収集14 + 申し込み状況7）のコスト。申し込み状況取得は別システムだが、同一プロジェクト内で稼働。
...
| シナリオ | 現在（21ジョブ） | 最大（47ジョブ） | 増加額 |
|---------|----------------|----------------|-------|
| Cloud Scheduler（ファイル収集） | $1.40 | $4.00 | +$2.60 |
| Cloud Scheduler（申し込み状況） | $0.70 | $0.70 | $0 |
| Cloud Run（ファイル収集） | $0.60 | $1.71 | +$1.11 |
| Cloud Run（申し込み状況） | $0.30 | $0.30 | $0 |
| **合計** | **$3.00** | **$6.71** | **+$3.71 (+124%)** |

**注**: 最大47ジョブ = ファイル収集40 + 申し込み状況7（別システム、変更なし）
...
| 10.5 | HTTPリクエストボディ | JSONパラメータ（class_name, task_id, task_pattern, drive_folder_id, spreadsheet_id） |
