...
#
# 注記（2026-08-26）: class_name/drive_folder_id/spreadsheet_idは意図的に「令和7年度」の
# ままにしています（既に実機検証済みの安全な組み合わせ、209件全件重複判定・新規書込みなしを
# 確認済み）。年度・保存先はハードコードのため、令和8年度の疎通確認に使う場合は
# 実行前に値を書き換えること（他クラス・他年度の未検証の保存先へ流用しない）。
#
