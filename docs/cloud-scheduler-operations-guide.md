...
> ⚠️ **2026-04-03に本ドキュメント対象のファイル収集ジョブは全てPAUSED（課金停止）**。以下は令和7年度運用当時の構成。**別システムの「申し込み状況取得用」ジョブ（`carewell-automation-pattern*`）はこの停止措置の対象外で、ENABLEDのまま稼働継続している**（2026-08-26確認）。令和8年度（2026年度）の最新状況は `docs/SERVICE_SHUTDOWN_AND_RESUME.md` を参照。

**合計25ジョブ**（学生同期含む）が稼働していた：
- **ファイル収集用**: 令和7年度16ジョブ（8クラス × 2課題）← 本ドキュメントの対象。**令和8年度は2026-08-26に10クラス（№06・07追加）へ拡大済み、20ジョブ**（`docs/SERVICE_SHUTDOWN_AND_RESUME.md`参照）
- **申し込み状況取得用**: 8ジョブ（8クラス）⚠️**別システム・絶対削除禁止**。2026-08-26確認時点で**10ジョブ（`carewell-automation-pattern1〜10`）へ増加しており、ENABLEDで稼働中**（本ドキュメント作成時から2件増えている。おそらく姉妹リポジトリ`carewell-moushikomi-csv`側での№06・07追加に伴うもの）
...
- **ジョブ数**: 8ジョブ（8クラス、pattern1-5, 8-10）
- **実行間隔**: 30分ごと
- **対象Cloud Run**: `carewell-automation`
- **用途**: Carewellから申し込み状況を自動取得
- **スプレッドシートID**: `1ZhSDpgxsC0NRkZy2cCmTBqYnKsZ90KRDc8UrKuodQMQ`
- **重要**: 本ドキュメントの対象外。このジョブは**絶対に削除・停止しないこと**
...
- [ ] Cloud Run `carewell-file-collector`が正常に稼働している

**申し込み状況取得システム（7ジョブ）⚠️絶対削除禁止:**
- [ ] 全7ジョブ（pattern1-5, 8-9）がENABLED状態である
- [ ] Cloud Run `carewell-automation`が正常に稼働している
- [ ] スプレッドシート（ID: 1ZhSDpgxsC0NRkZy2cCmTBqYnKsZ90KRDc8UrKuodQMQ）が正常に更新されている
...
#### 現在の構成（合計21ジョブ、24時間稼働）

**Schedulerジョブ料金:**
```
課金対象ジョブ数: 21 - 3 (無料枠) = 18ジョブ
月額: 18ジョブ × $0.10 = $1.80 (約270円)
...
全ジョブの実行回数: 1,440回 × 21ジョブ = 30,240回/月
...
申し込み状況取得（7ジョブ）:
  実行時間: 平均20秒/実行 × 10,080実行 = 201,600秒/月 = 56時間/月
  料金: 約 $0.30/月

合計Cloud Run料金: 約 $0.80/月 (約120円)
※ 無料枠（月2百万vCPU秒、360,000 GB秒）あり
```

**合計（現在21ジョブ、24時間稼働）:**
```
Scheduler: $1.80 (約270円)
Cloud Run: $0.80 (約120円)
合計: $2.60/月 (約390円/月)

内訳:
  - ファイル収集システム: 約¥240/月
  - 申し込み状況取得システム: 約¥150/月
```
...
#### シナリオ1: 現在の構成（合計21ジョブ、24時間稼働）

| 項目 | 詳細 | 月額（USD） | 月額（円） |
|------|------|------------|-----------|
| Scheduler ジョブ | 21ジョブ - 3無料 = 18ジョブ | $1.80 | ¥270 |
| Scheduler 実行 | 30,240回/月（無料枠内） | $0.00 | ¥0 |
| Cloud Run (ファイル収集) | 168時間/月、2GB | $0.50 | ¥75 |
| Cloud Run (申し込み状況) | 56時間/月、2GB | $0.30 | ¥45 |
| **合計** | | **$2.60** | **¥390** |

**内訳:**
- ファイル収集システム（16ジョブ）: ¥274/月
- 申し込み状況取得システム（7ジョブ）: ¥150/月
...
実行回数: 32回/日 × 30日 × 16ジョブ = 15,360回/月（無料枠内）
...
#### 推奨3: 将来的な拡張（ファイル収集40ジョブ）への準備

**現在のコスト（合計21ジョブ）:** ¥390/月
**将来のコスト（ファイル収集40 + 申し込み状況7）:** ¥915/月

**増加分:** ¥525/月（ファイル収集26ジョブ増加）

**準備事項:**
- 予算確保（年間¥11,000程度）
- スケジュール設計（5分刻みで最大12ジョブ/30分まで配置可能 → 24ジョブ/時間）
- 47ジョブ（40+7）の場合、一部を60分間隔に変更する必要あり
- **重要**: 申し込み状況取得の7ジョブは変更しないこと（別システム）
...
| 構成 | 月額 | 年額 | 備考 |
|------|------|------|------|
| 現在（合計21ジョブ、24h） | ¥390 | ¥4,680 | ファイル収集14 + 申し込み7 |
| ファイル収集のみ（16ジョブ、24h） | ¥274 | ¥3,288 | 本システムのみ |
| 最大（ファイル収集40 + 申し込み7） | ¥915 | ¥10,980 | 将来拡張時 |
...
- **現在稼働中**: ¥390/月
  ...
  - 申し込み状況取得システム（7ジョブ）: ¥150/月
- **将来拡張時（40+7ジョブ）**: ¥915/月
  - ファイル収集システム（40ジョブ）: ¥765/月
  - 申し込み状況取得システム（7ジョブ）: ¥150/月

**結論:**
- 現在の総コスト（¥390/月）は非常に低く、最適化の優先度は低い
- 将来的に40ジョブに拡張しても年間約¥11,000と許容範囲内
- **推奨: 現状の24時間稼働を継続**
...
現在21ジョブ（ファイル収集14 + 申し込み状況7）、最大47ジョブ（ファイル収集40 + 申し込み状況7）を想定しているため、制限には十分余裕があります。
...
令和8年度20ジョブのスケジュール配置（2026-08-26時点、全PAUSED。№06・07追加済み）：

| 時刻 | Cron式 | 配置ジョブ |
|------|--------|-----------|
| XX:00, XX:30 | `0,30 * * * *` | class01-task01, class04-task01, class09-task01, **class07-task01** |
| XX:05, XX:35 | `5,35 * * * *` | class01-task02, class04-task02, class09-task02, **class07-task02** |
| XX:10, XX:40 | `10,40 * * * *` | class02-task01, class05-task01, class10-task01 |
| XX:15, XX:45 | `15,45 * * * *` | class02-task02, class05-task02, class10-task02 |
| XX:20, XX:50 | `20,50 * * * *` | class03-task01, class08-task01, **class06-task01** |
| XX:25, XX:55 | `25,55 * * * *` | class03-task02, class08-task02, **class06-task02** |

> 📌 №06は`20,50`/`25,55`スロットへ、№07は`0,30`/`5,35`スロットへ合流（2026-08-26確定）。新規4ジョブは既存ピアジョブ（class03-task01）の実行設定（attemptDeadline等）を複製して作成した。将来resumeする際は、同一時刻に同時実行されるジョブが最大4件になる点、および別システムの`carewell-automation-pattern*`（10ジョブ、ENABLED稼働中）との時刻重複も踏まえ、一括ではなく段階的に有効化すること。
