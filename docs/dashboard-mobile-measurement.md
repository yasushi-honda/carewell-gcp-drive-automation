...
### 5.2 グループ一覧の提出状況表示（PR #48・#49、2026-09-20）

グループ一覧のカードに、その課題の提出状況（提出/未提出・合格/採点待ち/不合格）をバーと数字で出す機能。マージ後に本番（№01 課題①）で実測した。

**表示内容の照合（合格）**: 全15グループのカードが、Firestore（REST で件数のみ取得）から求めた期待値と完全に一致した。提出合計 13（13人・14ファイル、1人が2回提出）、未提出合計 241（active 254人 − 13）。全員「採点待ち」で実データと整合。名簿外の提出は0で、警告は出ない。390 / 768 / 1280px で横スクロールなし・はみ出し 0・全カードの高さ 188px で揃う。

**表示時間（課題カードをタップしてグループ一覧が出るまで、slow4g・CPU4倍・390px、5回）**

| 時点 | カードの初回表示（中央値、範囲） | バーの表示（中央値） | 判定 |
|---|---|---|---|
| 変更前（提出状況なし） | 約350〜380ms | — | 基準 |
| PR #48 マージ後（提出データを受講生と同時に取得） | 507ms（453〜543） | 507ms | **未達**（目標 ≤450ms） |
| PR #49 マージ後（受講生の取得後に提出データを取得） | **398ms**（391〜562） | 471ms | 合格 |

- PR #48 で未達になった原因は、提出データを同時に取り始めたため、約35KB が同じ回線を分け合ってカードの表示が約130ms 遅れたこと（通信量の増加と遅れの大きさが合うことからの推測。切り分けの実験はしていない）。PR #49 でカードの取得後に提出データを取る形に変え、カードが先に出てバーが約70ms 後に埋まる形になった。
- 変更前の「約350〜380ms」は別スクリプト（3回）の値で、今回の398msとの差20〜50msは測定の揺れの範囲か、提出状況の追加による増分かを分けていない。5回のうち1回目だけ562ms（バーは640ms）の外れ値があった。
- **提出が増えたときは未検証**: 実測は現在の14ファイルまで。提出データは1件約2.5KBで、全員提出（約254件）なら約630KB。カードの表示は先に出る設計だが、バーの表示は slow4g で約+3秒遅れる見込み（実測していない）。取得量の削減（REST の項目指定、BE での集計）は今回のスコープ外。
- **合否は未検証**: 合否付きの実データがまだ無い（全件 `pass_status` 空・「採点待ち」）。「合格」「不合格」の表示と複数回提出の扱い（最新の提出の状態）は、最初の採点結果が入った後に提出元の画面と照合するまで未確認。
...
**追補（2026-09-20・PR #56 の本番確認）**: グループ内受講生の一覧に「提出状況」の列を足したため、表の最小幅が約812pxから約1029pxに広がった。本番（グループ R・№01 課題①）で実測すると、1024px では表（1029px）が表示領域（896px）に収まらず、右端の提出状況の列が横スクロールしないと見えなかった。表が収まるのは約1157px（表示領域 = 画面幅 − 128px の余白）から。そのため切替を 1024px から `/students` と同じ 1280px に変更した（1280px では表示領域 1152px に対して表 1029px で収まる）。
- 1280px 未満: カード、1280px 以上: 表。上の段階3の表のうち「グループ内受講生」の境界（1023 / 1024）は、この追補で 1279 / 1280 に置き換わる。
- 教訓: 表に列を足したときは、切替幅の前提（表の最小幅）が変わるため、実測して境界を見直す。
