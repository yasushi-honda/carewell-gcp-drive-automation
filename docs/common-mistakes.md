...
## Incident #16: External File Dependency Before Self-Check (2026-09-11)

### 📅 発見日時
2026-09-11

### 🚨 症状
- Issue #18対応中、令和8年度№01の出欠管理ファイルに「受講者リスト」タブ（日介番号⇔受講者番号のマッピング）を構築する必要が生じた
- クライアントから届いたのは中間データ2種（グループ分け結果254件、日介番号なし／クライアント側作業用ファイルの日介番号名簿254件）のみで、完成データが揃っていなかった
- AI（Claude）が氏名+ふりがなキーで手動マージし静的に貼り付けたが、この延長で「クライアント側の作業用ファイル（一時的性質）へIMPORTRANGEで永続依存する」設計を検討しかけた

### 🔍 根本原因
- クライアントから届いたデータが不完全だった際、真っ先に「クライアントにもっと良い形式のデータを依頼する」「クライアント側ファイルに数式で依存する」という外向きの解決策を検討した
- 自プロジェクト群内に、同じ日介番号情報を独立管理している既存の公式データソース（`carewell-moushikomi-csv`プロジェクトの「申込状況」スプレッドシート、jaccw自社の公式申込受付システム）があることを、外部依存を検討するより先に確認していなかった

### ✅ 解決策・検証結果
1. `carewell-moushikomi-csv/CLAUDE.md`から「【申込状況】令和8年度」スプレッドシートID（`1ybWm0n0e7Ixk6A_GbLGwmPhixza01PF7Kn0NXv6jXcg`）とTeam1〜10=№01〜10の対応を特定
2. Team1タブ（№01相当）の氏名・ふりがな・日介番号298件を取得し、クライアント側データ254件とNFKC正規化（+空白除去+カタカナ→ひらがな変換）した氏名+ふりがなキーで突合
3. **254/254件で日介番号が完全一致**（独立した2系統のデータソースが完全に整合）。単純なNFKC正規化のみでは246/254件、全角/半角空白の統一で253/254件、ひらがな/カタカナ表記ゆれの吸収で254/254件に到達——表記ゆれの種類ごとに正規化を積み上げて初めて全件一致が判明した
4. 唯一の残存差異は1件のふりがな表記ゆれ（「りゆうすけ」/「りゅうすけ」）のみで、日介番号自体は一致

### 🎯 教訓
- **クライアントへのデータ依頼・クライアント側ファイルへの数式依存を検討する前に、まず自プロジェクト群内で既に保有・管理している公式データソースがないかを確認する**
- 自社管理データソースは、クライアント側の一時的な作業用ファイルより安定性・継続性の点で優れる（削除・再構成リスクがない、権限管理が自社完結）
- ただし過剰な調査は不要——「他に確認すべき自社内ソースの候補があるか」を一度立ち止まって考える程度で十分。今回は`GOAL.md`に以前のセッションで「carewell-moushikomi-csvに日介番号データがある」と記録済みだったため、grep一発で発見できた
- 氏名+ふりがなでの名寄せは、全角/半角空白・ひらがな/カタカナ等の表記ゆれで見かけ上「不一致」になりやすい。突合率が100%に届かない場合、即座に「別人/データ欠落」と判断せず、表記ゆれの可能性を先に疑う（ただし表記ゆれの自動吸収を無条件に信用せず、最終的には日介番号等の不変IDで裏付けを取る）

### 📊 影響分析
- 対象: №01（254名）、今後残り7クラス（№02,03,04,05,06,07,09。№08,10はスコープ外・9/24以降に再確認）にも同型の判断が必要
- データへの影響: なし（検証のみ、書き込みは別途decision-maker承認後）

### 🔗 関連ファイル
- `carewell-moushikomi-csv/CLAUDE.md`「令和8年度シーズン運用状況」節
- 本リポジトリ `docs/handoff/GOAL.md`（過去セッションでの申込データソース候補の記録）

### 今後の改善
- №01残り8件の不一致原因を特定
- 残り7クラス（№02〜10、うちスコープ内は№02〜07・09）についても同様の突合を実施し、`carewell-moushikomi-csv`側データを日介番号の正式ソースとして採用するかをdecision-makerと確定する

---

## Incident #17: Timeout Misdetected as Confirmed Zero Submissions (2026-09-11)

### 📅 発見日時
2026-09-11

### 🚨 症状
- Cloud Runのログで各クラスの提出0件が継続していたが、AI（Claude）はこれを「実データがまだ存在しない」と即断し、decision-makerに報告した
- decision-makerから「実際に取得対象先のページをPlaywrightで目視したか」と直接指摘され、ログのみに基づく未検証の断定だったことが発覚
- Secret Managerからの認証情報取得が安全分類器でブロックされたため、decision-manager自身がPlaywrightブラウザで手動ログインし、AIが直接ページを確認したところ、確かに実提出0件（「提出されたレポートはありません。」表示）ではあったが、その結論に至った経路（`src/playwright_automation.py`の`get_submission_list()`）自体に潜在バグがあることが判明した

### 🔍 根本原因
- `get_submission_list()`のStep 3（`wait_for_selector("tr.standard_grid_item", timeout=60000)`）は、提出0件のページでは対象要素自体が存在しないため必ずタイムアウトする
- このタイムアウト例外を汎用的に捕捉し「0件」として扱っていたため、**「本当に0件」と「取得失敗（ページ構造変化・ネットワーク不調等）で0件に見えているだけ」を区別できない**設計になっていた
- この「success・0件の誤検知」リスクは計画書（plan-crossreview時点）で既知リスクとして記録されていたが、実装段階で対処が漏れていた

### ✅ 解決策・検証結果
1. ゼロ件ページが表示する正規メッセージ「提出されたレポートはありません。」の文字列を`CarewellSelectors.NO_SUBMISSIONS_TEXT`として定義
2. Step 2（tbody待機）成功後・Step 3（行要素待機）実行前に、このメッセージの有無を`get_by_text(..., exact=False).count() > 0`で先にチェックする分岐を追加。メッセージが検出できた場合のみ「確定済み0件」として扱い、Step 3のタイムアウト待機自体をスキップする
3. `py_compile`・`black --check`で構文・整形確認、`codex review --base main`でdiscrete correctness issuesなしを確認
4. PR #28としてマージ・Cloud Runへデプロイ後、次回スケジュール実行（2026-09-11 11:32 UTC）のログで、№01・№04・№07・№09全てで新しいログ`✓「提出されたレポートはありません」を検出。提出0件が確定しました(取得失敗によるタイムアウトではありません)`が正しく発火することを本番ログで直接確認した

### 🎯 教訓
- **ログの結果（"0件"等の数値）だけで実世界の状態を断定しない。可能な場合は必ず直接ソースを確認する**（decision-maker指摘: 「これは本当ですか？実際に取得対象先のページをPlaywrightで目視しましたか？」）。間接証拠（ログメッセージ）と直接証拠（実際のページ内容）を混同しない
- **「タイムアウト＝異常」と「タイムアウト＝正常の一形態（対象要素が存在しないケース）」を区別する設計にする**。要素が存在しないことを確認する専用の待機・検出ロジック（正規のゼロ件メッセージの検出等）を用意し、汎用的なtry/exceptで両者を一括して同じ結果に丸め込まない
- 計画段階で「既知リスク」として記録されていても、実装時に見落とされることがある。デプロイ後は本番ログで実際の分岐が意図通り発火しているかまで確認して初めて「直った」と言える（デプロイ成功≠新コード実行中＝パターン5と同型）

### 📊 影響分析
- 対象: 全クラス共通ロジック（`src/playwright_automation.py`の`get_submission_list()`）。№01に限らず、稼働中の全クラス（№01,04,07,09含む）で同じ誤検知リスクがあった
- データへの影響: 提出0件という判定結果自体は変わらない（実際に0件だったため）。ただし「本当に0件」と「取得失敗」を区別できないままだった場合、将来ページ構造変化等で取得が実際に失敗した際に気づけないリスクがあった

### 🔗 関連ファイル
- `src/playwright_automation.py`（`CarewellSelectors.NO_SUBMISSIONS_TEXT`、`get_submission_list()`）
- PR #28

### 今後の改善
- 同様の「要素待機タイムアウト＝結果ゼロ」という設計パターンが他の抽出ロジックにも存在しないか、次回のコードレビュー時に確認する
