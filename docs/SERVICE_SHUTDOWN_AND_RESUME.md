...
1. `src/config/classes.py`の`STUDENT_SPREADSHEET_IDS_BY_YEAR`に令和8年度の正しい名簿スプレッドシートIDを設定するまで（または`STUDENT_SPREADSHEET_ID`環境変数で明示指定するまで）、新サイトの「同期」ボタンは押さない（未設定のままだと`ValueError`で失敗する設計のため、誤った年度の名簿が使われることはないが、動作もしない）
