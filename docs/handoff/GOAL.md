...
- [ ] Step 2: 残り7クラス（№02,03,04,05,06,07,09）のIMPORTRANGE+XLOOKUP+COUNTIF数式書込み（№02は「受講者リスト」データ整備済みだが提出記録シートに「課題①」タブ未作成＝提出0件のため保留。他6クラスは先方からのグループ分けデータ到着待ち、下記中断点参照）
...
- （2026-09-20）**Dashboard のモバイル対応**（スマホでヘッダーが縦に崩れる・トップの表示が遅い・受講生の表が横スクロール、の報告）を4段階で対応しマージ・本番実測済み。PR #42（計測ゲート）・#43（クラス一覧の Firestore 取得を並列化。本番の表示 1331→359ms、41→12リクエスト）・#44（ヘッダー。390px で高さ202→112px、320px の横スクロール解消）・#45（受講生一覧・グループ内受講生の表を狭い画面でカード表示。画面外要素 1537→0 / 60→0）。実測値は `docs/dashboard-mobile-measurement.md` §5.1
  - 切替幅: ヘッダーは 1024px、`/students` は 1280px（表の最小幅が約1012pxのため）。グループ内受講生は当初 1024px だったが、PR #56 で提出状況の列を足して表の最小幅が約1029pxに広がったため、PR #57 で 1280px に変更した（下記の提出状況表示のエントリ参照）。44px のタップ領域は 1024px 未満のみ（この点は decision-maker 承認済み）。ヘッダーの1024px境界と `/students` の1280pxは実測に基づく実装側の判断で、PR本文で開示のうえマージされた（事前の個別承認ではない）
...
- （2026-09-20）**受講生一覧の「通し番号」列を「受講者番号」（A001 形式）に置き換え**（decision-maker の質問「通し番号が全員 `-` なのは問題か」がきっかけ）。原因は令和8年度の出欠管理名簿に通し番号の列が無く、新経路（PR #33）が `serial_number` を常に 0 で書いていたこと。decision-maker の指摘で、名簿の「受講者番号」（グループをまたいだ連番）が通し番号に相当すると確認し、すでに同期済みの `student_number` の表示・並べ替えに切り替えた。PR #52（受講生一覧・グループ別一覧・モバイルのカード）。本番で全画面を確認（254人全員に A001〜R254、降順切替も動作）
  - 後始末（PR #54、2026-09-20）: `requirements.md`・`docs/phase2-verification-checklist.md` に注記、表側の受講者番号列のテストを追加、管理者ログイン時のヘッダーを本番で確認（390px・320px とも崩れなし）
  - 未着手（decision-maker の指示待ち）: BE が書き続ける `serial_number` / `student_serial_number`（常に 0）の整理。旧年度（令和7年度）の経路は本物の通し番号を書くため、整理すると旧年度の表示を壊すおそれがある
  - （2026-09-25 解消）Issue #18 の名簿投入後の受講者番号表示を、№02（249名、Firestore同期経由）で実機確認済み。Dashboard `/students` で阿久津剛（A001）〜横溝しをり（R249）まで249名全員が正しい受講者番号・グループ分類で表示されることをPlaywright MCPで確認。残り6クラス（№03,04,05,06,07,09）も名簿投入後に同じ確認を行う
- （2026-09-20）**グループ内の受講生一覧に「提出状況」を表示**（decision-maker の要望「受講生一覧のビューで、誰が課題を提出しているか・ステータスが分からず、1人ずつ開かないといけないのは不便」）。PR #56（各受講生の未提出/合格/採点待ち/不合格 + 最終提出日時、状態別の人数チップで絞り込み。取得失敗・名簿不一致は「全員未提出」と誤表示せず警告、退会者は在籍者のみ集計し提出のない退会者は「対象外」、提出データは受講生の取得後に取得）と PR #57（本番確認で、列追加により 1024px で「提出状況」列が画面外になる不具合を検出 → 表とカードの切替を 1280px に変更）。本番で A・R・B グループがグループ一覧カードと一致、1024/1279px はカード・1280px は表で提出状況が画面内に収まることを確認済み
  - 未検証: 合格・不合格の表示は実データが無くユニットテストのみ。条件待ち（trigger: №01 課題①に最初の採点結果が入る）で、グループ一覧のカードとこの一覧を提出元の画面と照合する
- （2026-09-20）**パンくずの「課題①」が課題全体のファイル一覧に飛ぶ不具合を修正**（decision-maker の指摘）。PR #59: グループ一覧・グループ内受講生一覧のパンくずで、課題名を文字だけの目印にした（原因は、課題カードの遷移先がグループ一覧に変わった後もリンク先が `FileListView` のままだったこと）。本番で確認済み。導線のなくなった `FileListView`・ルート `files` は decision-maker の判断で残す（削除・導線追加はどちらも未実施）
- （2026-09-25）**carewell-file-collectorのCloud Runエラー緊急通知メールを自動化**（decision-maker依頼「問題があったら緊急通知メールを」）。調査の結果`scripts/setup-monitoring.sh`（2025-10-13、通知チャネル・ログベースメトリクスのみ実装済み、アラートポリシー作成はGCPコンソール誘導のまま放置）と、同じGCPプロジェクト内で稼働中の別アプリ（`carewell-moushikomi-csv`推測）向けアラート3件（同じ通知チャネル`carewell-email-notification`=hy.unimail.11@gmail.com使用）を発見。ゼロから作らずスクリプトのステップ4として完成させる形でPR #62をマージ
  - plan-crossreview（grip+codex 2パス）で監視範囲・PII混入・通知チャネルのfail-close化・E2E手順を精査し改訂。実装後もcodex review 4巡で計3件の実バグを検出・修正: ①`severity`フィールドがこのアプリのプレーンテキストログでは機能せず（Cloud Runは非構造化ログのログレベルを自動解析しない）テキストパターン一致方式に変更 ②一度検討した「Table wait failed」等の除外は、実コード確認の結果2026-09-11の確定0件判定修正（`confirmed_zero_submissions`）により既に本物のタイムアウト失敗のみを意味するようになっていたため撤回 ③fail-close検証失敗時にDRY_RUN=falseでもexit 1しない不具合を修正
  - 本番でアラートポリシーを作成し独立再検証・E2E疎通確認（合成ログ書込み→フィルタ完全一致）まで実施済み。メール受信そのものの最終確認は受信者本人（decision-maker）に依頼中、未確認
  - 副産物: 過去30日のログ調査で「提出0件」タイムアウト誤検知（2026-09-11修正）以前のノイズが17,314件/30日相当あったことを実データで確認、修正後は297件/14日（うち「Table wait failed」系が68件、いずれも本物のタイムアウト失敗）まで正常化していることも副次的に確認できた
  - スコープ外: `setup-monitoring.sh`の既存3アラート（高エラー率・連続失敗・実行時間超過）の完成、Cloud Scheduler/Sheets同期/CI失敗の通知は今回のdecision-maker選択で対象外のまま
