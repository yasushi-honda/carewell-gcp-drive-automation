...
class GoogleDriveService:
    ...
    def _initialize_service(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize Google Drive service: {e}", exc_info=True
            )
            raise
    ...
    def check_folder_access(self, folder_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully accessed folder: {folder.get('name')} (ID: {folder_id}, Type: {drive_type})"
            )

            # Warn if not in Shared Drive
            if not folder.get("driveId"):
                logger.warning(
                    f"Folder {folder_id} is in My Drive. Service accounts cannot upload to My Drive folders. Use Shared Drive instead."
                )
