...
- [ ] Verify database name is resolved via `resolve_firestore_database_id()` (NOT a hardcoded literal, NOT `(default)`)
...
| 17 | Timeout Misdetected as Confirmed Zero | 2026-09-11 | 提出0件をログのみで断定しかけた | ✅ **ログの数値だけで断定せず直接ソースを確認。タイムアウト＝異常とタイムアウト＝正常(要素不在)を区別**（詳細: `docs/common-mistakes.md` #17） |
...
**パターン9: タイムアウトと正常系（対象不在）の混同**
- インシデント #17（提出0件検知）
- 事象: 提出0件のページは対象要素（`tr.standard_grid_item`）自体が存在しないため、行要素の`wait_for_selector`が必ずタイムアウトする。このタイムアウトを汎用的に捕捉し「0件」として扱っていたため、「本当に0件」と「取得失敗で0件に見えているだけ」を区別できなかった。ログの"0件"表示のみで実データの不在を断定し、decision-makerに指摘されて発覚
- 教訓: **ログの数値だけで実世界の状態を断定せず、可能な場合は直接ソースを確認する。「タイムアウト＝異常」と「タイムアウト＝正常の一形態（対象要素が存在しないケース）」を区別する設計にし、正規のゼロ件メッセージ等で先に判定してからタイムアウト待機に入る**
