...
// 通し番号・ふりがなの並べ替えは「昇順 ⇄ 降順」の2状態（受講生一覧の3状態とは別仕様）。
...
const DATA: Student[] = [
  student('N03', { furigana: 'う', serial_number: 2 }),
  student('N01', { furigana: 'あ', serial_number: 3 }),
  student('N02', { furigana: 'い', serial_number: 1 }),
  student('N09', { furigana: 'え', serial_number: 4, group: 'B' }),
  student('N08', { furigana: 'お', serial_number: 5, class_name: 'No2' }),
];
...
describe('GroupStudentsView (responsive table / cards)', () => {
  ...
  describe('並べ替え（カード表示。昇順 ⇄ 降順の2状態）', () => {
    ...
    it('should toggle serial number between ascending and descending', async () => {
      const wrapper = await mountView(390);
