 ...
 *
 * ⚠️ 重要:
 * - この配列は Dashboard ホームページ（`/`）のクラス一覧カード表示に使用されます
 * - Cloud Run側の収集処理（`carewell-file-collector`）はこの配列を参照しません。
 *   収集処理は各Cloud SchedulerジョブのHTTPリクエストボディ（class_name等）に直接駆動されるため、
 *   このリストはあくまで Dashboard 表示・保守スクリプトの走査対象に過ぎません。
 * - `/admin/sync-students-from-sheets` API を実行しても、この配列は自動更新されません
 * - 新しいクラスを追加する場合は、手動でこのファイルを編集 → Git commit → デプロイが必要です
 ...
 *
 * Background:
 * - submissions コレクション: フルネーム形式 (例: "令和8年度 デジタル中核人材養成研修 №01")
...
export const CLASS_NAME_MAPPING: Record<string, string> = {
  ...
  '令和8年度 デジタル中核人材養成研修 №06': 'No6',
  '令和8年度 デジタル中核人材養成研修 №07': 'No7',
  '令和8年度 デジタル中核人材養成研修 №08': 'No8',
  '令和8年度 デジタル中核人材養成研修 №09': 'No9',
  '令和8年度 デジタル中核人材養成研修 №10': 'No10',
};

/**
 * フルネーム形式のクラス名を短縮形に変換
 *
 * @param fullClassName - フルネーム形式のクラス名 (例: "令和8年度 デジタル中核人材養成研修 №01")
 ...
 * @returns フルネーム形式のクラス名 (例: "令和8年度 デジタル中核人材養成研修 №01")、マッピングが存在しない場合は元の値を返す
 */
